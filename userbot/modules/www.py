...
async def pingme(pong):
    ...
    start = datetime.now()
    await pong.edit("🐣")
    await pong.edit("__**...🐣Last🐣...**__")
    await pong.edit("__**.....USERBOT.....**__")
    await pong.edit("__**......TUNGGUIN BNTR BANG......**__")
    await pong.edit("**0% ▒▒▒▒▒▒▒▒▒▒**")
    await pong.edit("**20% ██▒▒▒▒▒▒▒▒**")
    await pong.edit("**40% ████▒▒▒▒▒▒**")
    await pong.edit("**60% ██████▒▒▒▒**")
...
async def pingme(pong):
    ...
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    await pong.edit("`.....🐣Last🐣.....`")
    await pong.edit("`🐣`")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await pong.edit(f"•⎚• ⎆ __Last__ **Pong!**\n"
                    f">>> 𝗣𝗜𝗡𝗚:__ "
                    f"`%sms` \n"
                    f">>>  __Sisa Waktu:__ "
                    f"`{uptime}` \n" % (duration))
...
async def pingme(pong):
    ...
    duration = (end - start).microseconds / 1000
    await pong.edit(f"- Last -\n"
                    f"**👉🏼 sinyal  :** "
                    f"`%sms` \n"
                    f"**👉🏼 i'm online  :** "
                    f"`{uptime}` \n"
                    f"__|━|⎆__ **Bang  :** `{ALIVE_NAME}`" % (duration))
...
async def pingme(pong):
    ...
    uptime = await get_readable_time((time.time() - StartTime))
    start = datetime.now()
    await pong.edit("`.....👉🏼Last-Userbot👈🏼.....`")
    end = datetime.now()
    duration = (end - start).microseconds / 1000
    await pong.edit(f"**LAST PONG**\n"
                    f"➥__Ping:__ "
                    f"`%sms` \n"
                    f"➥__Uptime:__ "
                    f"`{uptime}` \n" % (duration))
...
async def pingme(pong):
    ...
    await pong.edit("⎚⎆ __Last__ **Test Ping!**\n`%sms`" % (duration))
